...
You are building the **personal-server-ts** (Phase 2: Auth + Builder Read Path).
...
- `docs/260128-phase-2-implementation-plan.md` — **The implementation plan (source of truth for tasks)**
...
Open `docs/260128-phase-2-implementation-plan.md` and select the **next eligible task**:
...
In `docs/260128-phase-2-implementation-plan.md`, change the completed task's status from `[ ]` to `[x]`.
...
```bash
git add -A
git commit -m "phase-2: ${TASK_ID} ${SHORT_DESCRIPTION}"
```
...
6. **Commit message format:** `phase-2: ${TASK_ID} ${short description}` (e.g., `phase-2: 0.2 scope wildcard matching`)
