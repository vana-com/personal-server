...
describe('timestampToFilename', () => {
  it('replaces colons with hyphens', () => {
    expect(timestampToFilename('2026-01-21T10:00:00Z')).toBe('2026-01-21T10-00-00Z')
  })
})

describe('filenameToTimestamp', () => {
  it('replaces hyphens in time portion with colons', () => {
    expect(filenameToTimestamp('2026-01-21T10-00-00Z')).toBe('2026-01-21T10:00:00Z')
  })
})
...
describe('buildDataFilePath', () => {
  it('builds correct path for two-segment scope', () => {
    expect(buildDataFilePath('/data', 'instagram.profile', '2026-01-21T10:00:00Z')).toBe(
      '/data/instagram/profile/2026-01-21T10-00-00Z.json',
    )
  })
  ...
  it('includes 3 directory segments for three-segment scope', () => {
    ...
    expect(path).toBe('/data/chatgpt/conversations/shared/2026-01-21T10-00-00Z.json')
  })
})

describe('buildScopeDir', () => {
  it('builds correct directory path', () => {
    expect(buildScopeDir('/data', 'instagram.profile')).toBe('/data/instagram/profile')
  })
})
