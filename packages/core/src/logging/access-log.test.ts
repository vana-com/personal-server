...
describe('AccessLogWriter', () => {
  ...
  it('file content is valid JSON line', async () => {
    ...
    const content = await readFile(join(logsDir, 'access-2026-01-28.log'), 'utf-8')
  ...
  it('two writes same day produce same file with 2 lines', async () => {
    ...
    await writer.write(makeEntry({ logId: 'log-1', timestamp: '2026-01-28T10:00:00Z' }))
    await writer.write(makeEntry({ logId: 'log-2', timestamp: '2026-01-28T22:00:00Z' }))
  ...
  it('writes to different days produce separate files', async () => {
    ...
    await writer.write(makeEntry({ logId: 'day1', timestamp: '2026-01-28T12:00:00Z' }))
    await writer.write(makeEntry({ logId: 'day2', timestamp: '2026-01-29T12:00:00Z' }))
