...
function isValidVerifyBody(body: unknown): body is VerifyRequestBody {
  ...
  if (typeof b.signature !== 'string' || !b.signature.startsWith('0x')) return false;
  ...
  if (typeof p.builder !== 'string' || !p.builder.startsWith('0x')) return false;
...
export function grantsRoutes(deps: GrantsRouteDeps): Hono {
  ...
  app.post('/verify', async (c) => {
    ...
    try {
      ...
      return c.json({ valid: false, error: 'EIP-712 signature verification failed' });
    }

    if (!valid) {
      return c.json({ valid: false, error: 'Grant signature does not match user' });
    }
